    ...
    class Program
    {
        static void Main(string[] args)
        {
            do
            {
                ...
                double priceOrigin = Convert.ToDouble(Console.ReadLine());

                Console.WriteLine("Enter trade price per share:");
                double priceTrade = Convert.ToDouble(Console.ReadLine());

                Console.WriteLine("Enter transaction type (Buy or Sell):");
                string transType = Console.ReadLine();
                Console.WriteLine(new string('-', 30));

                CurrentValueCalc inst = new CurrentValueCalc(nominal, priceOrigin, priceTrade, transType);
